...
- `summary` 用 30～60 字写一段高密度的人格速写，概括这类旅行者的判断方式、偏好和节奏，不能空泛。
...
- `strengths` 写 2～3 条这类人格在旅行中的天然优势，每条 14～28 字，要有判断价值。
- `watchouts` 写 2～3 条需要提醒的地方，每条 14～28 字，要具体，不要空泛说教。
- `bestScenarios` 写 3～4 个适合这类人格的旅行场景或方式，使用短词或短语。
- `actionTips` 写 2～3 条可直接执行的出行建议，每条 14～28 字，要像真正能采用的建议。
- `nextTripInspiration` 给出简短、可执行、与人格相符的下一次旅行灵感。
- `musicRecommendation` 只推荐一首适合该人格旅行时听的真实音乐或纯音乐；不确定具体曲目时可用稳定、广为人知的曲目。`title` 写“曲名 - 作者/歌手”，`mood` 写 2～6 字氛围词，`reason` 18～36 字说明为什么适配，不要展开乐评。
- `travelPrescription` 写一句 18～36 字的下一次旅行处方，必须具体可执行。
- `souvenirLine` 写一句 12～24 字的文艺短句，适合作为报告底部或分享图落款。
...

{
  ...
  "profile_data": {
    ...
    "strengths": ["优势 1", "优势 2"],
    "watchouts": ["提醒 1", "提醒 2"],
    "bestScenarios": ["场景 1", "场景 2", "场景 3"],
    "actionTips": ["建议 1", "建议 2"],
    ...
    "travelPrescription": "18～36 字具体旅行处方",
    "souvenirLine": "12～24 字报告落款短句",
    "visualTheme": "固定枚举之一"
  }
}
